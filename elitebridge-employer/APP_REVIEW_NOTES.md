# Elite Bridge Employer — App Review Notes (Version 1.2.0, Build 28)
...
Reviewers may also select **Create employer account** on the first screen. Registration is public, requires the password to be entered twice, and creates a real employer account using the same process available to every care organization. A time-limited activation link is sent to the new account's work email; administrative signup-email delivery happens in the background and does not hold the user on the registration screen.
...
3. **Employer sign in:** accepts only employer-role accounts. An organization’s authorized administrator uses an employer account to manage its workspace.
4. **Home:** displays open shifts, assigned shifts, team and applicant totals, a visible notification entry point, and the primary Publish Shift action.
...
8. **Team:** builds a live employee directory from caregivers assigned by this employer, including credentials, assignment totals, upcoming shifts and contact actions.
9. **Time & attendance:** displays real clock-in and clock-out activity recorded by caregivers on this employer’s assigned shifts.
10. **Notifications:** displays persistent application, claim, call-out, and account activity and supports read/unread state.
11. **Organization profile:** edits the employer’s real production organization name, contact information, services and service address.
12. **Account:** exposes organization profile, notifications, push settings, support, Privacy Policy, Terms of Use, sign out, and account deletion through **Advanced account options → Continue to account deletion**, followed by a destructive confirmation dialog to prevent accidental deletion.
